...
import { createPeerIdFromPublicKey, ENR, MAX_RECORD_SIZE, createNodeId, SignableENR, bytesToBigint, } from "@chainsafe/enr";
...
import { ConnectionStatusType, toResponseType, } from "./types.js";
import { RateLimiter } from "../rateLimit/index.js";
import { multiaddrFromSocketAddress, isEqualSocketAddress, multiaddrToSocketAddress, setSocketAddressOnENR, getSocketAddressOnENR, getSocketAddressMultiaddrOnENR, } from "../util/ip.js";
import { createDiscv5Metrics } from "../metrics.js";
...
export class Discv5 extends EventEmitter {
    ...
    static create(opts) {
        ...
        const sessionService = new SessionService(fullConfig, decodedEnr, keypair, transport ?? new UDPTransportService({ bindAddrs, nodeId: decodedEnr.nodeId, rateLimiter }));
     ...
     */
    async sendFindNode(remote, distances) {
    ...
    pingConnectedPeers() {
        for (const entry of this.kbuckets.rawValues()) {
            if (entry.status === EntryStatus.Connected) {
                this.sendPing(entry.value).catch((e) => log("Error pinging peer %o: %s", entry.value, e.message));
            }
        }
    }
    /**
    ...
    sendRpcRequest(activeRequest) {
        this.activeRequests.set(bytesToBigint(activeRequest.request.id), activeRequest);
                    ...
                    case InsertResult.Inserted: {
                        ...
                        this.connectedPeers.set(nodeId, setInterval(() => {
                            ...
                            if (this.kbuckets.getValue(nodeId)) {
                                this.sendPing(newStatus.enr).catch((e) => log("Error pinging peer %o: %s", newStatus.enr, e.message));
                            }
                            else {
                                clearInterval(this.connectedPeers.get(nodeId));
    ...
    onPendingEviction = (enr) => {
        this.sendPing(enr).catch((e) => log("Error pinging peer %o: %s", enr, e.message));
    };
    ...
    handleRpcResponse = (nodeAddr, response) => {
        ...
        if (requestNodeAddr.nodeId !== nodeAddr.nodeId || !requestNodeAddr.socketAddr.equals(nodeAddr.socketAddr)) {
            ...
            activeRequest.callbackPromise?.reject(new CodeError("Received a response from an nexpected address", ResponseErrorType[ResponseErrorType.WrongAddress]));
            return;
        }
        ...
        if (!requestMatchesResponse(activeRequest.request, response)) {
            ...
            activeRequest.callbackPromise?.reject(new CodeError("Response has incorrect response type", ResponseErrorType[ResponseErrorType.WrongResponseType]));
            return;
        }
        try {
            switch (response.type) {
                case MessageType.PONG:
                    this.handlePong(nodeAddr, activeRequest, response);
                    break;
                case MessageType.NODES:
                    if (!this.handleNodes(nodeAddr, activeRequest, response))
                        return;
                    break;
                case MessageType.TALKRESP:
                    this.handleTalkResp(nodeAddr, activeRequest, response);
                    break;
                default:
                    // TODO Implement all RPC methods
                    return;
            ...
            activeRequest.callbackPromise?.reject(new CodeError(e.message, ResponseErrorType[ResponseErrorType.InternalError]));
        }
    };
    ...
    handleNodes(nodeAddr, activeRequest, message) {
        ...
        if (message.total > 1) {
            ...
            if (currentResponse.count < 5 && currentResponse.count < message.total) {
                ...
                this.activeRequests.set(bytesToBigint(message.id), activeRequest);
