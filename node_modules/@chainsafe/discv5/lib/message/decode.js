...
export function decodeRequestId(rlpRaw) {
...
function decodeFindNode(data) {
    ...
    const distances = rlpRaw[1].map((x) => (x.length ? Number(bytesToBigint(x)) : 0));
    return {
...
function decodeNodes(data) {
    ...
    return {
        ...
        id: decodeRequestId(rlpRaw),
        total: rlpRaw[1].length ? Number(bytesToBigint(rlpRaw[1])) : 0,
...
function decodeTicket(data) {
    ...
    return {
        ...
        id: decodeRequestId(rlpRaw),
        ticket: rlpRaw[1],
        waitTime: rlpRaw[2].length ? Number(bytesToBigint(rlpRaw[2])) : 0,
    };
}
